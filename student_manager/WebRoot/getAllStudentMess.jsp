  ...
  <body>
   ...
   <tr>
     <th>课程ID</th>
      ...
      <%
        List<String[]> studentInfoList=(List<String[]>)request.getAttribute("studentInfoList");
        if(studentInfoList==null){
        DbUtil.setSpan(2);
        Pages pages=new Pages();
        int nowPage=pages.getCurPage();
        String sql="select *from stuinfo";
        String sqlPage="select count(*) from stuinfo";
        int totalPage=DbUtil.getTotalPages(sqlPage);
        pages.setSqlString(sql);
        //记住当前总页数
        pages.setTotalPage(totalPage);
      studentInfoList=DbUtil.getPageContent(nowPage, sql);
       DbUtil.setSpan(2);//设置每页显示的信息
       session.setAttribute("pages", pages);
       request.setAttribute("studentInfoList",  studentInfoList);
       }
       %>
       <c:forEach var="str"  items="${ studentInfoList }">
       <c:choose>
       <c:when test="${i%2==0 }">
     ...
     <c:if test="${curPage>1 }">
     <a href="studentInfoPageChange?curPage=${curPage-1}">上一页</a>
     </c:if>
     <form action="studentInfoPageChange "method="post">
     ...
     <a href="studentInfoPageChange?curPage=${curPage+1 }">下一页</a>
     </c:if>
       </td>
       </tr>
 </table>
